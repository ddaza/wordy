...
| `segments` | Finalized captions published to playback and search |
...
Known whole nonspeech markers are suppressed. Ordinary parenthesized speech is preserved. Rounded ends are bounded by the decoded window; malformed/out-of-window job output is rejected before replacing a checkpoint.
...
- `CaptionRecoveryTests` covers contained phrases, multiple-caption anchors, one-to-one duplicate consumption, punctuation, known silence, pending-tail recovery, EOF, migration, billing preservation, and a synthetic two-hour fold.
