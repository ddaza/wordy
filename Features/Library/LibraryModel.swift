...
final class LibraryModel {
    ...
    var selectedLecture: Lecture? { lectures.first { $0.id == selection } }
    ...
    private func importAudio(_ urls: [URL]) async {
        ...
        var failed = 0
        for url in urls {
            if let existing = lectures.first(where: { $0.url == url }) { selection = existing.id; continue }
            do {
                let audio = try await importer.inspect(url)
                let lecture = Lecture(title: audio.title, url: audio.url, duration: audio.duration,
                                      timeline: try TranscriptTimeline(segments: []))
                lectures.append(lecture)
                selection = lecture.id
            } catch { failed += 1 }
        }
        if failed > 0 { errorMessage = "Could not import \(failed) file(s). Choose an accessible file containing playable audio." }
    }

    func showSample() {
        if let sample = lectures.first(where: \.isSample) { selection = sample.id; return }
        let segments = [
