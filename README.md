...
- Caption boundaries remain saved but provisional until the following section arrives. That tail is shown immediately as approximate captions for playback and search. Distinct overlapping phrases are preserved with approximate timing. Existing checkpoints with complete raw results repair locally on reopen, with a backup and no retranscription or uploads.
