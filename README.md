...
- Caption boundaries remain saved but provisional until the following section arrives. That tail is shown immediately for playback and search. Distinct overlapping phrases keep their source times; the player shows one caption at a time. Existing checkpoints with complete raw results repair locally on reopen, with a backup and no retranscription or uploads.
