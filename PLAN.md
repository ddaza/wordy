...
Chunking isolation (2026-09-19): Each checkpoint stores the engine `raw` lists. Chunks no longer drop phrases by owned time; `CaptionPipeline` feeds raw to the stitch in order. Functional tests fold `Tests/Fixtures/*.json`. The development clip (`Tests/Fixtures/clip-14-20/`) records gold as overlapping 30 s+3 s Whisper windows vs cloud 60 s+10 s. See `docs/caption-pipeline.md`.
